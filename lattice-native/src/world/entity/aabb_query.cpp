...
using AabbScanSoaFn = void (*)(const double*, std::size_t,
                               const double*, std::size_t, std::size_t,
                               std::uint64_t*) noexcept;

std::atomic<AabbScanFn> g_aabb_scan{&aabb_scan_scalar};
std::atomic<AabbScanSoaFn> g_aabb_scan_soa{&aabb_scan_soa_scalar};
...
void init_aabb_dispatch() noexcept {
    ...
    AabbScanSoaFn soa_fn = &aabb_scan_soa_scalar;
    ...
    if (f.neon) {
        fn = &aabb_scan_neon;
        soa_fn = &aabb_scan_soa_neon;
    }
#endif

    g_aabb_scan.store(fn, std::memory_order_release);
    g_aabb_scan_soa.store(soa_fn, std::memory_order_release);
...
void aabb_scan_soa(const double* query_aabbs, std::size_t query_count,
    ...
    g_aabb_scan_soa.load(std::memory_order_acquire)(
        query_aabbs, query_count, entity_aabbs, entity_count, entity_stride, visibility);
}
