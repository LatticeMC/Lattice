...
void aabb_scan_avx2(const double* query_aabbs, std::size_t query_count,
    ...
    for (std::size_t q = 0; q < query_count; ++q) {
        ...
        for (std::size_t ce = 0; ce < full_chunks; ++ce) {
            ...
            const __m256d cmpXa = _mm256_cmp_pd(qMinX, eMaxX, _CMP_LE_OQ);
            const __m256d cmpXb = _mm256_cmp_pd(qMaxX, eMinX, _CMP_GE_OQ);
            const __m256d cmpYa = _mm256_cmp_pd(qMinY, eMaxY, _CMP_LE_OQ);
            const __m256d cmpYb = _mm256_cmp_pd(qMaxY, eMinY, _CMP_GE_OQ);
            const __m256d cmpZa = _mm256_cmp_pd(qMinZ, eMaxZ, _CMP_LE_OQ);
            const __m256d cmpZb = _mm256_cmp_pd(qMaxZ, eMinZ, _CMP_GE_OQ);
