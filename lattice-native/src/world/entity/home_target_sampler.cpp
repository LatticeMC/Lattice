...
[[nodiscard]] bool collides_with_obstacle(const HomeTargetInputs& in,
...
HomeTargetResult sample_home_target(const HomeTargetInputs& in) noexcept {
    HomeTargetResult result{};
    ...
    for (std::size_t i = 0; i < in.candidate_count; ++i) {
        ...
        const double candidate_home_sq = sq_distance(x, y, z, in.home_x, in.home_y, in.home_z);
        const double candidate_self_sq = sq_distance(x, y, z, in.self_x, in.self_y, in.self_z);
        const double distance_error = std::abs(candidate_home_sq - preferred_sq);
