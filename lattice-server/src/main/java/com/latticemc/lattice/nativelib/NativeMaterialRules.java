...
public final class NativeMaterialRules implements AutoCloseable {
    ...
    public void evaluateBatchInto(int count,
                                  int[] columnCtx,
                                  double surfaceNoise,
        ...
        checkOpen();
        if (count <= 0) return;
        if (columnCtx == null || columnBools == null || blockData == null || output == null) {
            throw new IllegalArgumentException("null batch context array");
        }
        ...
        if (output.length < count) throw new IllegalArgumentException("output array too short");
        if (namedNoiseValues == null) namedNoiseValues = EMPTY_DOUBLES;
        nativeEvaluateBatchInto(handle, count, columnCtx, surfaceNoise, surfaceSecondaryNoise, namedNoiseValues, columnBools, blockData, output);
    }
    ...
    private static native void nativeEvaluateBatchInto(long handle, int count, int[] columnCtx, double surfaceNoise, double surfaceSecondaryNoise, double[] namedNoiseValues, byte[] columnBools, int[] blockData, int[] output);
